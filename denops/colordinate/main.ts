...
class ColordinateBuffer {
  ...
  public static init = async (denops: Denops, name: string, cmd?: string) => {
    ...
    await autocmd.group(denops, "colordinate", (helper) => {
      ...
      helper.define(
        ["BufReadPost", "BufWinEnter", "TextChanged", "InsertLeave"],
        "<buffer>",
        `call denops#notify("${denops.name}", "reflect", [v:false])`,
      );
      helper.define(
        "BufWriteCmd",
        "<buffer>",
...
export async function main(denops: Denops): Promise<void> {
  ...
  denops.dispatcher = {
    ...
    async reflect(clear: unknown) {
      ...
      if (clear) {
        await helper.execute(denops, "hi clear");
      }
      await helper.execute(
        denops,
        `
  ...
  await helper.execute(
    ...
    command! ColordinateReload call denops#notify("${denops.name}", "reflect", [v:true])
